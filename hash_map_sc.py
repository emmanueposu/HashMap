...
# Description: Part 1, implement the HashMap class using chaining.


from hm_include import (DynamicArray, LinkedList,
...
class HashMap:
    ...
    def put_light(self, key: str, value: object) -> None:
        """
        Serves the same functionality as put, except it does not
        call resize_table or update self._size. It is a helper function for
        find_mode.
        """
        hash_val = self._hash_function(key)
        index = hash_val % self._capacity
        ...
        for idx in range(self._capacity):
            if self._buckets[idx].length() != 0:
                for node in self._buckets[idx]:
                    hash_objects.append((node.key, node.value))
...
def find_mode(da: DynamicArray) -> (DynamicArray, int):
    """
    Takes a dynamic array as a parameter and return a tuple containing, a
    dynamic array comprising the mode value/s of the array, and an integer that
    represents the highest frequency.
    """
    map = HashMap(da.length())

    for idx in range(da.length()):
        map.put_light(da[idx], 1)

    keys_and_values = map.get_keys_and_values()
    # target is used for tracking the most frequent key, ties are ignored
    target = None

    for idx in range(keys_and_values.length()):
        if target is None:
            target = keys_and_values[idx]
        elif keys_and_values[idx][1] > target[1]:
            target = keys_and_values[idx]
    # most_freq is used for storing most frequent keys, ties are included
    most_freq = DynamicArray()

    for idx in range(keys_and_values.length()):
        if keys_and_values[idx][1] == target[1] and \
                keys_and_values[idx][0] != target[0]:
            most_freq.append(keys_and_values[idx][0])

    most_freq.append(target[0])

    return most_freq, target[1]
